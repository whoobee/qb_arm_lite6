...
def generate_ros2_control_params_temp_file(ros2_control_params_path, prefix='', add_gripper=False, add_bio_gripper=False, add_robotiq_arg85=False, ros_namespace='', update_rate=None, robot_type='xarm', use_sim_time=False):
    if ros_namespace or prefix or add_gripper or add_bio_gripper or add_robotiq_arg85 or update_rate:
        ...
        if add_gripper:
            ...
            gripper_control_params_path = os.path.join(get_package_share_directory('xarm_controller'), 'config', 'robotiq_arg85_controllers.yaml')
            # check file is exists or not
            if os.path.exists(gripper_control_params_path):
                with open(gripper_control_params_path, 'r') as f:
                    gripper_control_params_yaml = yaml.safe_load(f)
